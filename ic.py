...
def train(train_loader, model, criterion, embed_criterion, optimizer, epoch, writer,
...
def train_net(args, writer, interactive=False):
...
def test_net(args, writer, interactive=False):
    batch_size = args.batch_size
    num_workers = args.workers
    phase = args.phase
    ...
    single_model = ICNet(args.arch, args.classes, args.down, interactive=interactive)
    model = torch.nn.DataParallel(single_model).cuda()
